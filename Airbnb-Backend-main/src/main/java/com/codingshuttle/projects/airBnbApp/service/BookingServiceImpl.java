...
public class BookingServiceImpl implements BookingService{
    ...
    public String initiatePayments(Long bookingId) {
        ...
        booking.setBookingStatus(BookingStatus.PAYMENTS_PENDING); // keep only this
    ...
    public void capturePayment(Event event) {
        ...
        if ("checkout.session.completed".equals(event.getType())) {
            ...
            if (!"paid".equals(session.getPaymentStatus())) {
                log.warn(" Payment not completed yet. Status: {}", session.getPaymentStatus());
                return;
            }

            String sessionId = session.getId();
            log.info(" Looking for booking with session ID: {}", sessionId);

            // This will now work because CheckoutServiceImpl saved it
            Booking booking = bookingRepository.findByPaymentSessionId(sessionId)
                    .orElseThrow(() -> {
                        log.error(" Booking not found for session ID: {}", sessionId);
                        return new RuntimeException("Booking not found for session ID: " + sessionId);
                    });

            log.info(" Found booking: {}. Updating to CONFIRMED", booking.getId());
